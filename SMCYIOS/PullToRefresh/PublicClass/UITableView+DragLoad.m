...
#define HAS_INIT    @"has_init"
#define OLD_SCROLL_VIEW_TOP @"oldscrollViewTop"
#define OLD_SCROLL_VIEW_Bottom @"oldscrollViewBottom"
...
@property (nonatomic, assign) float oldScrollViewTop;
@property (nonatomic, assign) float oldScrollViewBottom;
...
@dynamic hasInit;
@dynamic oldScrollViewTop;
@dynamic oldScrollViewBottom;

- (BOOL)hasInit{
    NSNumber *boolNumber = objc_getAssociatedObject(self, HAS_INIT);
    return [boolNumber boolValue];
}

- (void)setHasInit:(BOOL)hasInit{
    NSNumber *boolNumber = [NSNumber numberWithBool:hasInit];
    objc_setAssociatedObject(self, HAS_INIT, boolNumber, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (float)oldScrollViewBottom{
    NSNumber *floatNumber = objc_getAssociatedObject(self, OLD_SCROLL_VIEW_Bottom);
    return [floatNumber floatValue];
}

- (void)setOldScrollViewBottom:(float)oldScrollViewBottom{
    NSNumber *floatNumber = [NSNumber numberWithFloat:oldScrollViewBottom];
    objc_setAssociatedObject(self, OLD_SCROLL_VIEW_Bottom, floatNumber, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (float)oldScrollViewTop{
    NSNumber *floatNumber = objc_getAssociatedObject(self, OLD_SCROLL_VIEW_TOP);
    return [floatNumber floatValue];
}

- (void)setOldScrollViewTop:(float)oldScrollViewTop{
    NSNumber *floatNumber = [NSNumber numberWithFloat:oldScrollViewTop];
    objc_setAssociatedObject(self, OLD_SCROLL_VIEW_TOP, floatNumber, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)initOldInsetSize:(float)top bottom:(float)bottom{
    if(self.hasInit == NO){
        [self setOldScrollViewTop:top];
        [self setOldScrollViewBottom:bottom];
        [self setHasInit:YES];
    }
}

- (float)getOldInsetTop{
    return [self oldScrollViewTop];
}

- (float)getOldinsetBottom{
    return [self oldScrollViewBottom];
}
...
{
    [self initOldInsetSize:self.contentInset.top bottom:self.contentInset.bottom];
