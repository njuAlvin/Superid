...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame])
    {
		...
		_arrowImage.frame = CGRectMake(25.0f, 10.0f, 30.0f, 55.0f);
...
- (void)dragTableDidScroll:(UIScrollView *)scrollView
{
    ...
    if (_state != DragTableDragStateLoading_ot && scrollView.isDragging)
    {
		...
		if (_state == DragTableDragStatePulling_ot && [self footerVisbleHeightInScrollView:scrollView] < LOADMORE_TRIGGER_HEIGHT + [_delegate getOldinsetBottom] && !_loading)
        {
			[self setState:DragTableDragStateNormal_ot];
		}
        else if (_state == DragTableDragStateNormal_ot && [self footerVisbleHeightInScrollView:scrollView] > LOADMORE_TRIGGER_HEIGHT + [_delegate getOldinsetBottom] && !_loading)
        {
...
- (void)dragTableDidEndDragging:(UIScrollView *)scrollView
{
	...
	if (footerVisibleHeight >= LOADMORE_TRIGGER_HEIGHT + [_delegate getOldinsetBottom] && !_loading)
    {
...
//添加初始的bottom
		scrollView.contentInset = UIEdgeInsetsMake([_delegate getOldInsetTop], 0.0f, LOADMORE_TRIGGER_HEIGHT + contentInsetHeightAdder + [_delegate getOldinsetBottom], 0.0f);
...
//    UIEdgeInsets edgeInset = UIEdgeInsetsFromString(edgeInsetString);
    UIEdgeInsets edgeInset = UIEdgeInsetsMake([_delegate getOldInsetTop], 0.0f, [_delegate getOldinsetBottom], 0.0f);
