...
- (void)dragTableDidScroll:(UIScrollView *)scrollView
{
    if (_state != DragTableDragStateLoading_ot && scrollView.isDragging)
    {
		...
		if (_state == DragTableDragStatePulling_ot && scrollView.contentOffset.y > REFRESH_TRIGGER_HEIGHT - [_delegate getOldInsetTop] && scrollView.contentOffset.y < 0.0f && !_loading)
        {
			[self setState:DragTableDragStateNormal_ot];
		}
        else if (_state == DragTableDragStateNormal_ot && scrollView.contentOffset.y < REFRESH_TRIGGER_HEIGHT - [_delegate getOldInsetTop] && !_loading)
        {
...
- (void)dragTableDidEndDragging:(UIScrollView *)scrollView
{
	...
	if (scrollView.contentOffset.y <= REFRESH_TRIGGER_HEIGHT- [_delegate getOldInsetTop] && !_loading)
    {
		...
		scrollView.contentInset = UIEdgeInsetsMake(-REFRESH_TRIGGER_HEIGHT + [_delegate getOldInsetTop], 0.0f, [_delegate getOldinsetBottom], 0.0f);
...
- (void)endLoading:(UIScrollView *)scrollView shouldUpdateRefreshDate:(BOOL)shouldUpdate shouldChangeContentInset:(BOOL)shouldChangeContentInset
{
    if (_isLoading)
    {
        if (shouldChangeContentInset)
        {
            ...
            [scrollView setContentInset:UIEdgeInsetsMake([_delegate getOldInsetTop], 0.0f, [_delegate getOldinsetBottom], 0.0f)];
            [UIView commitAnimations];
        }
